@if (($dish->label_date() == null && $dish->toggle == 0) || ($dish->label_date() != null && $dish->labels->first() != null))
        <div class="dish clearfix @if($dish->sub_course->course->menu_id == $dish->drinkmenu())my-table row @endif @if($dish->image && !$dish->big_description($dish->id))pb-3 @endif">
            ...
            <div class="suggestion @if($dish->sub_course->course->menu_id == $dish->drinkmenu())col-2 @endif">@if ($dish->label_date() != null && $dish->labels->first() != null && $dish->labels->first()->pivot->price != '0.00'){{str_replace('.', ',', $dish->labels->first()->pivot->price)}}@elseif($dish->label_date() != null && $dish->labels->first() != null && $dish->labels->first()->pivot->price == '0.00') @elseif($dish->price !== '0,00'){{$dish->price}}@endif @if($dish->beer_id)<span class="dish-span"> / <i class="fa-solid fa-beer-mug-empty"></i> {{$dish->beer_id}} @endif @if($dish->wine_id)/ <i class="fa-solid fa-wine-glass"></i> {{$dish->wine_id}}</span>@endif</div>
